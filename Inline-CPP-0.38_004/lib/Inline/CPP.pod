...
C<int myfunc();> instead of C<int myfunc(void);>.  This is C++, not C.
...
The current "smoke test" pass rate seems to be around 85%.  I'm always
working on chipping away at that last 15%.  If you're one of the unfortunate
15% get in touch with me so we can try to figure out what the problem is.
