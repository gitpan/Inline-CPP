...
second or third snippet.  This is no problem for a C++ developer who knows his
target compiler.  But Perl runs just about everywhere.  If similar portability
is a design goal, Inline::CPP helps by providing two C<#define> constants that
may be checked to ascertain which style of headers are being used.  The
constants are:
...
As Inline currently requires Perl 5.6.0 or newer, and Inline::CPP depends on
Inline, Perl 5.6.0 is required for Inline::CPP too.  Historically 5.6.0 was
required to get all the "new" features, although Perl 5.005_03 was supported
with limited functionality up to Inline::CPP version 0.25.  It's hard to
imagine anyone still using a Perl older than 5.6.0 wanting to interface with
C++, but if you're in that camp, you'll need to roll back Inline and
Inline::CPP to older versions through the magic of backpan.
...
This example shows a class with a constructor defined inline. Inline::CPP
version 0.25 or older could parse this example with 5.005. But this example
requires Perl 5.6.0:
...
=head2 Advice on Perl versions

Inline::CPP no longer actively supports Perl versions prior to 5.6.0.  This
shouldn't be an issue for anyone because Inline and Inline::C also no longer
support Perl versions prior to 5.6.0.  If you are in the unlikely position
of having a code base that uses Inline::CPP, and operates under a Perl older
than 5.6.0, don't upgrade L<Inline>, L<Inline::C>, or Inline::CPP (unless you
intend to also upgrade to a version of Perl released in the 21st Century.)
...
Work through issues relating to successful installation and use on
